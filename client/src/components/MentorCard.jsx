...
export default function MentorCard({ mentor, onClick }) {
  ...
  return (
    <div className="card cursor-pointer hover:border-green-200 transition-colors" onClick={onClick}>
      <div className="flex items-start gap-3">
        <div className="w-10 h-10 bg-green-700 rounded-full flex items-center justify-center text-white text-sm font-bold shrink-0">
          {initials}
        </div>
        <div className="flex-1 min-w-0">
          ...
          {mentor.bio && <p className="text-xs text-gray-600 mt-1">{mentor.bio}</p>}
        </div>
